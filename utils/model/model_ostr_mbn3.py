...
class encode(nn.Module):
    def __init__(self, model):
        super().__init__()
        self.mbnet_layer1 = nn.Sequential(*(list(model.children())[0][:4])) # 1~3
        self.mbnet_layer2 = nn.Sequential(*(list(model.children())[0][4:7])) # 4~6
        self.mbnet_layer3 = nn.Sequential(*(list(model.children())[0][7:13])) # 7~12
        self.mbnet_layer4 = nn.Sequential(*(list(model.children())[0][13:]),
                                *(list(model.children())[1])) # 0: (13~15) + 1: (*)
...
class OSnet_mb_frozen(nn.Module):
    def __init__(self):
        ...
        self.encode_texture = Deep_Orientation(960, 2048, 512) # 2048, 2048, 512
        self.encode_texture1 = Deep_Orientation(960, 2048, 512) # 2048, 2048, 512
        ...
        self.decode2 = nn.Sequential(
            nn.Conv2d(1136, 512, 1), # 2048, 512, 1
            nn.BatchNorm2d(512), # 512
            nn.ReLU(inplace=True),
        )

        self.decode3 = nn.Sequential(
            nn.Conv2d(552, 256, 1), # 1024, 256, 1
            nn.BatchNorm2d(256), # 256
            nn.ReLU(inplace=True),
        )
        self.decode4 = nn.Sequential(
            nn.Conv2d(280, 512, 1), # 512, 512, 1
            nn.BatchNorm2d(512), # 512
            nn.ReLU(inplace=True),
...
class OSnet_mb_free(nn.Module):
    def __init__(self):
        super().__init__()
        self.encode = encode(model=mobilenet_v3_module(pretrain=True,size='large',freeze=False))
        self.encode1 = encode1(model=mobilenet_v3_module(pretrain=True,size='large',freeze=False))
        self.ca = ChannelAttention(in_planes=1024)
        # self.sa = SpatialAttention()
